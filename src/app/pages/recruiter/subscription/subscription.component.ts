  ...
  template: `
    <div class="subscription-container">
      <div class="header-section">
        <h1 class="page-title">Subscription & Limits</h1>
        <p class="page-subtitle">Choose a plan that fits your hiring needs</p>
      </div>
      ...
      <div class="current-status-card" *ngIf="currentSubscription">
        ...
        <div class="usage-stats">
          <div class="stat-item">
            <span class="label">Job Posts Used</span>
      ...
      <div class="pricing-grid">
        <div class="pricing-card" *ngFor="let plan of plans" [class.featured]="plan.featured">
          ...
          <button class="buy-button" (click)="initiatePayment(plan.type)" [disabled]="isCurrentPlan(plan.type) || isLoading">
            {{ isCurrentPlan(plan.type) ? 'Current Plan' : 'Upgrade Now' }}
          </button>
        </div>
      </div>
...
export class SubscriptionComponent implements OnInit {
  ...
  plans = [
    {
      ...
      name: 'Free',
      price: 0,
      features: ['10 Job Postings', 'Standard Support', 'Basic Analytics'],
      featured: false
    },
    {
      ...
      name: 'Basic',
      price: 49,
      features: ['20 Job Postings', 'Priority Email Support', 'Advanced Analytics', 'Resume Downloads'],
      featured: false
    },
    {
      ...
      name: 'Pro',
      price: 99,
      features: ['50 Job Postings', '24/7 Support', 'Expert Analytics', 'Featured Listings', 'Direct Messaging'],
      featured: true
    },
    {
      ...
      name: 'Premium',
      price: 199,
      features: ['Unlimited Job Postings', 'Dedicated Account Manager', 'Custom Branding', 'API Access', 'Automated Screening'],
      featured: false
    }
  ];
  ...
  loadCurrentSubscription(): void {
    ...
    this.isLoading = true;
    this.subscriptionService.getCurrentSubscription(this.userId, 'RECRUITER').subscribe({
      next: (sub) => {
  ...
  initiatePayment(plan: SubscriptionPlan): void {
    ...
    this.isLoading = true;
    this.subscriptionService.initiateSubscription(this.userId, 'RECRUITER', plan).subscribe({
      next: (response) => {
  ...
  openRazorpay(orderId: string, plan: SubscriptionPlan): void {
    const options = {
      key: 'rzp_live_Slehh7RyO9XnHC', // Use the provided live key
